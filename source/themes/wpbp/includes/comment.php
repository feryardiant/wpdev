...
class Comment {
	...
	public function form_default_fieds( $fields ) {
		...
		$fields['author'] = join( '', [
			...
			'<label class="label" for="author">' . __( 'Name', 'wpbp' ) . $req_html . '</label>',
			'<div class="control">',
		...
		$fields['email'] = join( '', [
			...
			'<label class="label" for="email">' . __( 'Email', 'wpbp' ) . $req_html . '</label>',
			'<div class="control">',
		...
		$fields['url'] = join( '', [
			...
			'<label class="label" for="url">' . __( 'Website', 'wpbp' ) . '</label>',
			'<div class="control">',
		...
		$fields['cookies'] = join( '', [
			...
			'<input ' . $consent . ' id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes">' . __( 'Save my name, email, and website in this browser for the next time I comment.', 'wpbp' ),
			'</label>',
			'</div></div>',
		] );
	...
	public function form_defaults( $args ) {
		...
		$args['comment_field'] = join( '', [
			...
			'<label class="label" for="comment">' . __( 'Comment', 'wpbp' ) . '</label>',
			'<div class="control">',
		...
		$args['must_log_in'] = join( '', [
			...
			sprintf(
				...
				__( 'You must be <a href="%s">logged in</a> to post a comment.', 'wpbp' ),
		...
		$args['logged_in_as'] = join( '', [
			...
			sprintf(
				...
				__( '<a href="%1$s" aria-label="%2$s">Logged in as %3$s</a>. <a href="%4$s">Log out?</a>', 'wpbp' ),
				get_edit_user_link(),
				/* translators: %s: user name */
				esc_attr( sprintf( __( 'Logged in as %s. Edit your profile.', 'wpbp' ), $user_identity ) ),
				$user_identity,
